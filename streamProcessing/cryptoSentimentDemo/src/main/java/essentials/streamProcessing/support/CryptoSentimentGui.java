...
public class CryptoSentimentGui {
    ...
    public CryptoSentimentGui(
        ...
        jetResults.addEntryListener(
                (EntryAddedListener<Tuple2<CoinType, WinSize>, Tuple2<Double, Long>>) this::onMapEvent,
                true
        );

        jetResults.addEntryListener(
                (EntryUpdatedListener<Tuple2<CoinType, WinSize>, Tuple2<Double, Long>>) this::onMapEvent,
                true
        );
    ...
    private void onMapEvent(EntryEvent<Tuple2<CoinType, WinSize>, Tuple2<Double, Long>> event) {
        EventQueue.invokeLater(() -> {
            ...
            double mentionsPerMinute = metrics.f1() / (
                    (double) winSize.durationMillis() /
    ...
    private void startGui() {
        JFreeChart sentimentChart = chart(
                "Cryptocurrency Sentiment in Tweets",
                "Sentiment Score",
                sentimentDataset,
                -1.0,
                1.0
        ...
        JFreeChart mentionsChart = chart(
                "Mentions of Cryptocurrency in Tweets",
                "Tweets per Minute",
                mentionsDataset,
                0,
                100
